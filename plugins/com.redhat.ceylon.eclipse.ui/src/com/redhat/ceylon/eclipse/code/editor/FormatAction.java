...
final class FormatAction extends Action {
	...
	public static boolean isEnabled(CeylonParseController cpc) {
		return cpc != null && cpc.getStage().ordinal() >= Stage.SYNTACTIC_ANALYSIS.ordinal()
	...
	public static void format(final CeylonParseController pc, IDocument document, final ITextSelection ts,
		...
		if (!formatAll) {
			...
			if (selectedRootNode instanceof Body || selectedRootNode instanceof CompilationUnit) {
				// format only selected statements, not entire body / CU (from
				// now on: body)
				...
				if (formattingUnits.isEmpty()) {
					// possible if the selection spanned the entire content of
					// the body,
					// or if the body is empty, etc.
					formattingUnits.add(new FormattingUnit(selectedRootNode, (CommonToken) selectedRootNode.getToken(),
							(CommonToken) selectedRootNode.getEndToken()));
				}
			} else {
				formattingUnits = Collections.singletonList(new FormattingUnit(selectedRootNode,
						(CommonToken) selectedRootNode.getToken(), (CommonToken) selectedRootNode.getEndToken()));
			}
		} else {
			// format everything
			formattingUnits = Collections.singletonList(
					new FormattingUnit(pc.getRootNode(), tokenList.get(0), tokenList.get(tokenList.size() - 1)));
		}
		...
		try {
			for (FormattingUnit unit : formattingUnits) {
				...
				final TokenSource tokens = new TokenSource() {
					...
					public Token nextToken() {
						if (i <= endTokenIndex)
							...
							return tokenList.get(tokenList.size() - 1); // EOF
																		// token
						else
							return null;
					}
				...
				format_.format(unit.node, combinedOptions_.combinedOptions(
						loadProfile_.loadProfile(CeylonStyle.getFormatterProfile(pc.getProject()),
								/* inherit = */ false, /* baseDir = */ pc.getProject().getLocation().toOSString()),
						new Singleton<SparseFormattingOptions>(SparseFormattingOptions.$TypeDescriptor$, wsOptions)),
						new StringBuilderWriter(builder), new BufferedTokenStream(tokens), indentLevel);
				if (unit == formattingUnits.get(0)) {
		...
		if (selected) {
			...
			text = builder.substring(0, builder.length() - wsOptions.getLineBreak().getText().length());
		} else {
