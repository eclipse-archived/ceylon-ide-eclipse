...
public class SourceMenuItems extends CompoundContributionItem {
	...
	private static ImageRegistry imageRegistry = CeylonPlugin.getInstance().getImageRegistry();
	...
	// private static ImageDescriptor FORMAT =
	// imageRegistry.getDescriptor(FORMAT_BLOCK);
	// private static ImageDescriptor ADD =
	// imageRegistry.getDescriptor(ADD_COMMENT);
	// private static ImageDescriptor REMOVE =
	// imageRegistry.getDescriptor(REMOVE_COMMENT);
	// private static ImageDescriptor TOGGLE =
	// imageRegistry.getDescriptor(TOGGLE_COMMENT);
	// private static ImageDescriptor CORRECT =
	// imageRegistry.getDescriptor(CORRECT_INDENT);
	// private static ImageDescriptor LEFT =
	// imageRegistry.getDescriptor(SHIFT_LEFT);
	// private static ImageDescriptor RIGHT =
	// imageRegistry.getDescriptor(SHIFT_RIGHT);
	...
	private IContributionItem[] getItems(IEditorPart editor) {
		return new IContributionItem[] {
				new DynamicMenuItem(PLUGIN_ID + ".action.cleanImports", "&Organize Imports",
				...
				new Separator(),
				new DynamicMenuItem(PLUGIN_ID + ".action.pasteAsCeylon", "&Paste Java as Ceylon", true),
				new Separator(),
				...
				new DynamicMenuItem(PLUGIN_ID + ".editor.formatSource", "&Format", true),
				new DynamicMenuItem(PLUGIN_ID + ".editor.formatBlock", "Format &Block", true), new Separator(),
				new DynamicMenuItem(ITextEditorActionDefinitionIds.SHIFT_LEFT, "Shift &Left", true),
				new DynamicMenuItem(ITextEditorActionDefinitionIds.SHIFT_RIGHT, "Shift &Right", true), new Separator(),
				new DynamicMenuItem(PLUGIN_ID + ".editor.toggleComment", "Toggle &Comment", true),
				new DynamicMenuItem(PLUGIN_ID + ".editor.addBlockComment", "&Add Block Comment", true),
				new DynamicMenuItem(PLUGIN_ID + ".editor.removeBlockComment", "Re&move Block Comment", true) };
	}
