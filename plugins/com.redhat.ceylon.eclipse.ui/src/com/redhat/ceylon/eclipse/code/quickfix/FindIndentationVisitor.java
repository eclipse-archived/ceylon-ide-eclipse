...
class FindIndentationVisitor extends Visitor {

    private int indentation;
    private int currentIndentation;
    private Tree.StringLiteral literal;

    FindIndentationVisitor(Tree.StringLiteral literal) {
    ...
    public void visit(Tree.StringLiteral that) {
        ...
        if (type != STRING_MID && type != STRING_END) {
            currentIndentation = that.getToken().getCharPositionInLine() + 
            		getStartQuoteLength(type);
        }
        if (that == literal) {
	...
	static String fixIndent(Tree.CompilationUnit cu, Tree.StringLiteral node, 
			String text, int offset, int length, String extraIndent) {
	    FindIndentationVisitor fiv = new FindIndentationVisitor(node);
	    fiv.visit(cu);
	    if (fiv.getIndentation() != -1 && 
	    		offset != -1 && length != -1) {
	        return getFixedText(text, fiv.getIndentation(), extraIndent);
	    }
	    else {
	    	return null;
	    }
    ...
    static String getFixedText(String text, int indentation, 
    		String extraIndent) {
    ...
    static int getStartQuoteLength(int type) {
