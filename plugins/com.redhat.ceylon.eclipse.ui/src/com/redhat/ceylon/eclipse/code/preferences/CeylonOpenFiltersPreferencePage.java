...
//    }
    ...
    private void addPackage() {
        try {
            ...
            dialog.setMessage("&Select a package to filter from dialog:"); 
...
//                fUseStepFiltersButton.getSelection());
        ...
        store.setValue(OPEN_FILTERS, pref);
        pref = serializeList(inactive.toArray(new String[inactive.size()]));
        store.setValue(INACTIVE_OPEN_FILTERS, pref);
...
//        setPageEnablement(true);
    ...
    protected Filter[] getAllStoredFilters(boolean defaults) {
        ...
        if (defaults) {
            activeString = store.getDefaultString(OPEN_FILTERS);
            inactiveString = store.getDefaultString(INACTIVE_OPEN_FILTERS); 
        }   
        else {
            activeString = store.getString(OPEN_FILTERS);
            inactiveString = store.getString(INACTIVE_OPEN_FILTERS);
        }
