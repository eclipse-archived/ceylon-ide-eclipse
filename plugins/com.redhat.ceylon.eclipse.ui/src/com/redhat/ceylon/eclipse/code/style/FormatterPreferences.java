...
public class FormatterPreferences {
    ...
    public String get(String key) {
        ...
        switch(key) {
            case FORMATTER_indentMode:
                ret = options.getIndentMode().getClass().getSimpleName().toLowerCase();
                break;
            case FORMATTER_indentMode_Spaces_Size:
            case FORMATTER_indentMode_Tabs_Size:
                ret = new Long(options.getIndentMode().getWidthOfLevel()).toString();
                break;
            case FORMATTER_indent_Blank_Lines:
                ret = options.getIndentBlankLines().booleanValue()?"true":"false";
                break;    
    ...
    public void put(String key, String value) {
        switch(key) {
            case FORMATTER_indentMode:
                IndentMode indentMode = getIndentMode(value,
                        new Long(options.getIndentMode().getWidthOfLevel()).intValue());
                options.setIndentMode(indentMode);
                break;
            case FORMATTER_indentMode_Spaces_Size:
            case FORMATTER_indentMode_Tabs_Size:
                String mode =  options.getIndentMode().getClass().getSimpleName().toLowerCase();
                indentMode = getIndentMode(mode, Integer.parseInt(value));
                options.setIndentMode(indentMode);
                break;
            case FORMATTER_indent_Blank_Lines:
                options.setIndentBlankLines(Boolean.parseBoolean(value)?new ceylon.language.true_():new ceylon.language.false_());             
                break;
            default :
                break;
        }
    ...
    public FormattingOptions getOptions() {
        return combinedOptions_.combinedOptions(new FormattingOptions(), 
                new Singleton<SparseFormattingOptions>(SparseFormattingOptions.$TypeDescriptor$, options));
    }
}
