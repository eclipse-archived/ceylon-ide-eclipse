...
/**
 * @author Akber Choudhry Utility query and update for style options
 * 
 */
public class CeylonStyle {
    ...
    public static String getnewModuleVersion(IProject project) {
        CeylonConfig config = readProjectStyle(project);
        if (config != null
                && config.isOptionDefined(PREF_STYLE_NEW_MODULE_VERSION)) {
    ...
    public static String getnewModuleAuthor(IProject project) {
        CeylonConfig config = readProjectStyle(project);
        if (config != null
                && config.isOptionDefined(PREF_STYLE_NEW_MODULE_AUTHOR)) {
    ...
    public static boolean setnewModuleVersion(IProject project,
    ...
    public static boolean setnewModuleAuthor(IProject project,
            String newModuleAuthor) {
        CeylonConfig options = readProjectStyle(project);
    ...
    // FIXME very inefficient to read one by one
    private static boolean writeProjectStyle(IProject project,
