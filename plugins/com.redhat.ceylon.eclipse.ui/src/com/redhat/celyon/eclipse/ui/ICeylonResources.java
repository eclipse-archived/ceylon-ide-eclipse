package com.redhat.celyon.eclipse.ui;

public interface ICeylonResources {

	// Definitions for label provider

	public static final String CEYLON_DEFAULT_IMAGE = "ceylon_default_image";

	public static final String CEYLON_DEFAULT_OUTLINE_ITEM = "ceylon_default_outline_item";

	public static final String CEYLON_FILE = "ceylon_file";

	public static final String CEYLON_FILE_WARNING = "ceylon_file_warning";

	public static final String CEYLON_FILE_ERROR = "ceylon_file_error";

	// Definitions for label provider end

}
